...
impl RunResult {
    ...
    pub fn json_validation_error(validation_errors: Vec<String>) -> Self {
        let array_json = match serde_json::to_string(&validation_errors).map_err(|e| Error::Io(e.to_string())) {
            Ok(json) => json,
            Err(e) => return Self::json_io_error(Error::Io(e.to_string())),
        };
        let json = format!("\"validation_errors\":{}", array_json);
        Self {
...
mod tests {
    ...
    fn test_json_validation_error() {
        ...
        assert_eq!(result.validation_errors[0], "\"validation_errors\":[\"file has multiple owners\"]");
    }
}
